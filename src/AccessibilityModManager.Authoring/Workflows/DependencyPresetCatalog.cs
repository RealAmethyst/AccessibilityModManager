...
public sealed class DependencyPresetDefinition
{
    ...
    public Dependency ToDependency() => Build();
    public override string ToString() => DisplayName;
}

public static class DependencyPresetCatalog
{
    public static IReadOnlyList<DependencyPresetDefinition> All { get; } = Array.AsReadOnly(
    [
        new DependencyPresetDefinition
        {
            Id = "emulator",
        ...
        new DependencyPresetDefinition
        {
            ...
            Build = BuildNet10Desktop
        },
        Net9Desktop(
            id: "dotnet-9-desktop-x64",
            bits: 64,
            registryArch: "x64",
            installerUrl: Net9X64Url,
            sha256: Net9X64Sha256),
        Net9Desktop(
            id: "dotnet-9-desktop-x86",
            bits: 32,
            registryArch: "x86",
    ...
    private static readonly IReadOnlyDictionary<string, DependencyPresetDefinition> PresetsById =
        All.ToDictionary(preset => preset.Id, StringComparer.OrdinalIgnoreCase);
    ...
    public static Dependency CreateDependency(string presetId)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(presetId);

        if (!TryCreateDependency(presetId, out var dependency))
        {
            throw new InvalidOperationException($"Dependency preset '{presetId}' was not found.");
        }
    ...
    public static bool TryGet(string presetId, [NotNullWhen(true)] out DependencyPresetDefinition? preset)
    {
    ...
    private static Dependency BuildPortableEmulator() =>
        new()
        {
            Id = "emulator",
            Type = "system",
    ...
    private static Dependency BuildMelonLoader() =>
        new()
        {
            Id = "melonloader",
            Type = "framework",
    ...
    private static Dependency BuildBepInEx() =>
        new()
        {
            Id = "bepinex",
            Type = "framework",
    ...
    private static Dependency BuildNet10Desktop() =>
        new()
        {
            Id = "dotnet-10-desktop",
            Type = "system",
    ...
    /// </summary>
    private static DependencyPresetDefinition Net9Desktop(
        string id,
        int bits,
        string registryArch,
        string installerUrl,
